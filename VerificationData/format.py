def formatThresholdOutput(row):
    NTCQ=row['NTCQ']
    SUM =row['SUM']
    ADD_MAP  = list(row['ADD_MAP'])
    CHARGEQ = list(row['CHARGEQ'])
...
#    print(NTCQ,nPadBits)
    ...
    return paddedData#def formatThresholdOutput(NTCQ, ADD_MAP, CHARGEQ,SUM,MOD_SUM_0,MOD_SUM_1,MOD_SUM_2):
def formatThresholdOutput(row):
    NTCQ=row['NTCQ']
    SUM =row['SUM']
    ADD_MAP  = list(row['ADD_MAP'])
